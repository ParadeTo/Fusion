pro TSFusionAss,Ref_files,F_files,OutPutPath,QuaArr,Message=Message
  ;描述：
  ;时空融合质量评价
  ;
  ;
...
;Example
;    F_files = 'D:\Fusion\Data\HJ\2011\corrected\6_NDVI\hj1a-ccd1-447-56-20110811-l20000592938_albers105_sub_geo_sub_NDVI-tif.tif'
;    Ref_files= 'D:\Fusion\Data\HJ\2011\corrected\6_NDVI\hj1a-ccd1-447-56-20110811-l20000592938_albers105_sub_geo_sub_NDVI-tif.tif'
;    OutputPath = 'D:\'
;    QuaArr=['TRUE','TRUE','TRUE','TRUE']
  ...
  ;文件个数
  n_count = n_elements(Ref_files)
  
  for i_count=0,n_count-1 do begin
    ;;;;;;;;;;;;Open files;;;;;;;;;;;;;;;;;;;
    ;Open ref. image
    ENVI_OPEN_FILE,Ref_files[i_count],r_fid=ref_fid
    if (ref_fid eq -1) then begin
      Message = '无法打开'+Ref_files[i_count]
      envi_batch_exit
    endif
    
    ;Get parameters of ref. image
    envi_file_query,ref_fid,ns=ref_ns,nl=ref_nl,dims=ref_dims,nb=ref_nb
    ref_mapinfo=ENVI_GET_MAP_INFO(fid=ref_fid)
    ;Open fusion image
    ENVI_OPEN_FILE,f_files[i_count],r_fid=f_fid
    if (f_fid eq -1) then begin
      Message = '无法打开'+f_files[i_count]
      envi_batch_exit
    endif
    ;Get parameters of fusion image
    envi_file_query,f_fid,ns=f_ns,nl=f_nl,dims=f_dims,nb=f_nb
    f_mapinfo=ENVI_GET_MAP_INFO(fid=f_fid)
    ;获取数据
    RefData = fltarr(ref_ns,ref_nl,ref_nb)
    fData = fltarr(f_ns,f_nl,f_nb)
    FOR i_nb=0,ref_nb-1 DO BEGIN
      tempdata = ENVI_GET_DATA(DIMS=[-1L,0,ref_ns-1,0,ref_nl-1],FID=ref_fid,POS=i_nb)
      RefData[*,*,i_nb] = tempdata
      tempdata = ENVI_GET_DATA(DIMS=[-1L,0,f_ns-1,0,f_nl-1],FID=f_fid,POS=i_nb)
      fData[*,*,i_nb] = tempdata
    ENDFOR
    ;txt文件
    basename = file_basename(F_files[i_count],'.tif')
    OutFileTxt=OutputPath+basename+'_Qua.txt'
    openw,lun,OutFileTxt,/get_lun
    ;输出

    ENVI_REPORT_INIT,["Input File 1:"+F_files[i_count],"Input File 2:"+Ref_files[i_count],"Output File:"+OutFileTxt],$
      title="时空融合质量评价",base=base,/interrupt
    ENVI_REPORT_INC,BASE,1
    for i_nb = 0,ref_nb-1 do begin
      str = 'Band'+strtrim(string(i_nb+1),2)
      printf,lun,'-----------------------------------------'+str+'-----------------------------------------------------'
      TempRef = refdata[*,*,i_nb]
      TempF = fData[*,*,i_nb]
      index = where(TempRef ne 0 and TempRef lt 99999 and TempRef gt -99999 and TempF lt 99999 and TempF gt -99999 and TempF ne 0)
      TempRef = TempRef[index]
      TempF = TempF[index]
      if QuaArr[0] eq 'TRUE' THEN BEGIN
        printf,lun,'-----------------------Mean of Absolute Difference-----------------------------'
        index = where(TempRef ne 0 and TempF ne 0,count_index)
        Result=Moment(abs(TempRef[index]-TempF[index]))
        printf,lun,Result[0]
      endif
      ENVI_REPORT_STAT,BASE,30*(i_nb+1)/(ref_nb),100
      if QuaArr[1] eq 'TRUE' THEN BEGIN
        printf,lun,'-----------------------EA-----------------------------'
        ea = 1-sqrt(Result[1])/MEAN(abs(TempRef[index]))
        printf,lun,ea
      endif
      ENVI_REPORT_STAT,BASE,60*(i_nb+1)/(ref_nb),100
      if QuaArr[2] eq 'TRUE' THEN BEGIN
        printf,lun,'-----------------------Bias(Evaluated IMG-Referenced IMG)-----------------------------'
        Bias = total(TempF[index]-TempRef[index])/(n_elements(TempRef[index]))
        printf,lun,Bias
      endif
      ENVI_REPORT_STAT,BASE,80*(i_nb+1)/(ref_nb),100
      if QuaArr[3] eq 'TRUE' THEN BEGIN
        printf,lun,'-----------------------Linear Fitting Result-----------------------------'
        TempF1 = TempF[index]
        TempRef1 = TempRef[index]
        ;拟合
        ;Message='a'
        result = ScatterPlot(TempRef1,TempF1)
        ;R2
        Y = result[0]*TempRef1
        SEYMean = total((TempF1-mean(TempF1))*(TempF1-mean(TempF1)))
        SELine = total((TempF1-Y)*(TempF1-Y))
        R2 = 1-FLOAT(SELine)/SEYMean
        if R2 gt 0 then begin
          str ='y='+STRTRIM(string(result[0]),2)+'x'+'  '+'R2='+STRTRIM(string(R2),2)
        endif else begin
          str ='y='+STRTRIM(string(result[0]),2)+'x'+'  '+'Not significant'
        endelse
        printf,lun,str
      ;画散点图
      ;        TempRef1 = TempRef[index]
      ;        TempF1 = TempF[index]
      ;        n_points = n_elements(TempRef1)
      ;        times = 32000
      ;        index2 = indgen(n_points/times)*times
      ;        TempRef2 = TempRef1[index2]
      ;        TempF2 = TempF1[index2]
      ;        TempRef2=findgen(21)
      ;        TempF2=sin(findgen(21))
      ;        b = PLOT(TempRef2,TempF2,buffer=1)
      ;        b.SYMBOL="Circle"
      ;        b.LINESTYLE = 'NONE'
      ;        b.save,"d:\tt.jpg",border=10,resolution=300,/transparent
      ENDIF
      ENVI_REPORT_STAT,BASE,100*(i_nb+1)/(100.*ref_nb),100   
    endfor
...
function ScatterPlot,x,y
  ...
  start = [1]
  rerr=y*0.0+1.0
  result = MPFITEXPR(expr,x,y,rerr,start)
  return,result
end
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;以下代码非原创;;;;;;;;;;;;;;;;;;;;;;;;;;;;
